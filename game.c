...
void destruct_game(Game* game)
{
  ...
  _logf(L_INFO, "Game with state %s and limits %d | %d | %d | %d destructed\n",
        game->state ? "true" : "false", game->boats[1], game->boats[2], game->boats[3], game->boats[4]);
  free(game);
  return;
}
...
void print_game(Game* game)
{
  ...
  printf("Boats: %d | %d | %d | %d\n", game->boats[1], game->boats[2], game->boats[3], game->boats[4]);
  // printf("Board 1:\n");
